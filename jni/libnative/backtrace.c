...
#define ip2p(a) (ll2p((uint64_t)(a)))

static char* map2so(char* map_name) {
    char* so_name = null;
    if (map_name != null) {
        so_name = strrchr(map_name, '/');
        so_name = so_name != null ? so_name + 1 : map_name;
    }
    return so_name;
}
...
char* stacktrace_(bool detailed, siginfo_t* sig_info, void* sig_context, int from) { /* ~213 microseconds */
    ...
    *p = 0;
    ...
    for (i = from; i < (size_t)frame_count && n > 128; ++i) {
        ...
        if (!detailed) {
            if (name != null) {
                char* so_name = map2so(bs[i].map_name);
                if (so_name != null) {
                    k = snprintf(p, n, "%s%s %s+%d", p == buf ? "" : " <- ", so_name, name, pc);
                } else {
                    k = snprintf(p, n, "%s%s+%d", p == buf ? "" : " <- ", name, pc);
                }
            } else {
                ...
                if (symbol != null) {
                    ...
                    k = snprintf(p, n, "%s(%s%+d)[0x%p]",
                                p == buf ? "" : " <- ",
                                symbol->name,
                                (unsigned int)offset, ip2p(frames[i].absolute_pc));
                } else {
                    char* so_name = map2so(bs[i].map_name);
                    if (so_name != null) {
                        k = snprintf(p, n, "%s%s %p",
                                     p == buf ? "" : " <- ", so_name, ip2p(bs[i].relative_pc));
                    } else {
                        k = snprintf(p, n, "%s%p", p == buf ? "" : " <- ", ip2p(bs[i].relative_pc));
                    }
                }
            }
            p += k;
            ...
            if (bs[i].symbol_name != null) {
                ...
                if (symbol != null) {
                    uintptr_t offset = frames[i].absolute_pc - symbol->start;
                    k = snprintf(p, n, "%s%s (%s%+d)[%p]", p == buf ? "" : " <- ",
                                line, symbol->name, (int)offset, ip2p(frames[i].absolute_pc));
                    p += k;
                    n -= k;
                } else {
                    k = snprintf(p, n, "%s%s", p == buf ? "" : " <- ", line);
                    p += k;
                    n -= k;
                }
