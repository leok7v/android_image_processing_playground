...
#define posix_info(r) do { int _##r = (r); int e = errno; if (_##r != 0) { trace("ret=%d errno=%d \"%s\"", _##r, e, strerror(e)); } } while (false)
#else
#define posix_ok(r)   do { (void)(r); } while (false)
#define posix_info(r) do { (void)(r); } while (false)
#endif

void trace_(const char* file, int line, const char* func, const char* fmt, ...);
void vtrace_(const char* file, int line, const char* func, const char* fmt, va_list vl);
